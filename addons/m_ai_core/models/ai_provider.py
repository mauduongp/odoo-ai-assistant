...
class AiProvider(Model):
    ...
    model_ids = fields.One2many(
        "m_ai.model", "provider_id", string="Models", domain=[("active", "=", True)]
    )
    ...
    def get_provider(self, provider_type=None):
        ...
        if provider_type and provider.provider_type != provider_type:
            raise UserError(
                _("Configured AI Provider is of type '%s', but '%s' is required.")
    ...
    def get_response(self, prompt, system_prompt):
        ...
        if not hasattr(self, method_name):
            raise UserError(
                _("Method %s not implemented for provider %s")
                % (method_name, self.provider_type)
            )
        ...
        try:
            ...
            if res:
                return {
                    ...
                    "params": {
                        "title": _("Success"),
                        "message": _("Connection OK!"),
                        "type": "success",
                    },
                }
        except Exception as exc:
            raise UserError(_("Connection failed: %s") % str(exc))
